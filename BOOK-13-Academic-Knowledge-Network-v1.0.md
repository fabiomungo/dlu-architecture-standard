...
| Query facade | `kg_query_service`, GraphRAG + `frontier`/`gap` canonical forms — **STX-05** | ✅ | remaining canonical forms (prereq_chain, coverage, asset_needs) + budget probes 🟡 |
| Learner plane | BKT ✅; `kg_overlay_sync` batched consumers (KNOWS/EVIDENCES, DEFER-flush, tenant-scoped) — **delivered STX-05** | ✅ | WS03 rendering (STX-11) |
| Sync | `graph_sync_service_pi1` (+Competency/Framework definition tier), `kg_event_bus`, fire-and-forget; batched overlay consumers + `dlu_event_group_lag` metric — **STX-03/05** | ✅ | nightly drift reconciliation + `kg-rebuild` ⚪ |
...
| Drift/rebuild | export service | 🟡 | reconciliation job + `kg-rebuild` ⚪ (DR requirement) |
