...
| Map inputs | AKN ✅ (v2.0 🔵), `TeachingSection`/`AcademicTerm` ✅, ExternalCourse ✅, federation core 🟡, RouteGraph compiler ✅ (NEW-04 — `route_graph_compiler.py`: full seven-type edge taxonomy; `ENROLL` and now `RECOGNIZE` ✅ (STX-12 — `recognize_edges_for_twin`, a position effect not a weight, deliberately query-time not baked into the cached compile) live in production, `SIT_EXAM`/`RETAKE` a called seam for NEW-05, `CHALLENGE`/`MILESTONE` typed stubs, `FEDERATED` modeled+inactive; `route_graph_versions` persisted, every scenario cites its map version) | 🟡→🔵 | full learner-conditioned map still ⚪ pending NEW-05 (session edges) + K4 (federation policy) |
