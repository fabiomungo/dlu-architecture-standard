...
| Map inputs | AKN ✅ (v2.0 🔵), `TeachingSection`/`AcademicTerm` ✅, ExternalCourse ✅, federation core 🟡, RouteGraph compiler ✅ (NEW-04 — `route_graph_compiler.py`: full seven-type edge taxonomy, `ENROLL` live in production, `SIT_EXAM`/`RETAKE` a called seam for NEW-05, `RECOGNIZE`/`CHALLENGE`/`MILESTONE` typed stubs, `FEDERATED` modeled+inactive; `route_graph_versions` persisted, every scenario cites its map version) | 🟡→🔵 | full learner-conditioned map still ⚪ pending STX-12 (recognition inputs) + NEW-05 (session edges) + K4 (federation policy) |
| ETA | **correction (STX-07, repo-verified): `eta_service.py` is a generic job-completion estimator, not an academic model** — new graduation-ETA velocity model built as its own module | ✅ (STX-07, as new work) | confidence tier capped at low/medium (no "high" — needs a real calendar compiler; NEW-04 didn't add a calendar compiler, only the RouteGraph map — still ⚪) |
...
| Scenarios engine | `fastest_path` + path-health ✅ (STX-07); real multi-objective Pareto engine ✅ (NEW-04 — label-correcting, ≤8 non-dominated, deterministic, fixture-proven on all six weight dimensions); other 3 named scenarios still `not_yet_available` | 🔵→partial ✅ | `best_career_path`/`lowest_cost` ⚪ (STX-12 — no ESCO crosswalk/cost field exists anywhere yet); `highest_competency_growth` ⚪ for a DIFFERENT reason (NEW-04 finding, not an inputs gap — real competency data now populates `fastest_path.competency_gain`; blocked on elective-choice/credit-target modeling not existing, so a distinct scenario would be a fabricated relabeling) |
...
| Session-granular routing (G2) | `AssessmentSession` aggregate (BOOK-15) + ESSE3 appello mirror (BOOK-18 driver); `SIT_EXAM`/`RETAKE` edge types + compiler CALL SEAM ✅ (NEW-04 — `_sit_exam_edges`/`_retake_edges`, return `[]` today, NEW-05 fills the body not the call site) | ⚪→🔵(seam) | session calendars + live edge data ⚪ (NEW-05) |
| Traffic/invalidation | event mesh (STX-03), fingerprints (constitutional) — full BOOK-14 Ch. 6.1 traffic table ✅ (NEW-04): grade/assessment/competency/goal/risk wired + hysteresis (`PathHealth.replan_recommended`, >5% on the adopted scenario's own objective, or infeasibility) + `pending_verbalization` (G3) overlay, all real, tested | 🔵→✅ (6/8 rows) | 2 rows have NO mesh event and stay deferred by design: "recognition decision" (STX-12) and "policy/regulation change" (G7) — closing either needs a cross-repo taxonomy RFC, not attempted this sprint. "Offering/term change → batch replan" is answered WITHOUT an event: `trigger_batch_replan()`, called directly by whatever recompiles a program's map |
| Guidance surfaces | WS01 Academic Journey ✅ (STX-07 — Canvas/Companion/Missions triad, `/student/journey`), WS02 (STX-12), Navigator agent ✅ (STX-06 registered, STX-07 moves bound: `replan_path`/`explain_concept`/`reframe_goal`) | 🔵→partial ✅ | scenario-comparison Pareto-column UX (BOOK-17) — the engine exists (NEW-04) but only `fastest_path` has real numbers to compare; WS01 renders the rest as "coming" until STX-12 |
