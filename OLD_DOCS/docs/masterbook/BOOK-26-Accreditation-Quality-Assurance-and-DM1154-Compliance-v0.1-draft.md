...
>
> **✅ Affiliation exclusion (FW6-10, RFC-0001 §4.7.5, closes G18.6, 2026-08-08).**
> `FacultyProfile` gained `affiliation_type ∈ ('own','federated','visiting')`
> (additive, `NOT NULL DEFAULT 'own'` — no existing evaluation's verdict
> changes). `count_available_faculty` now EXCLUDES every profile whose
> `affiliation_type != 'own'` from the Dtot/Ttot figures above and reports
> them separately (`FacultyRequirementEvaluation.available_non_own_faculty`,
> also additive) — surfaced in the AVA UI itself, not only in the API
> payload. This closes the gap BOOK-22 §11.5(10) names: onboarding a faculty
> member seconded from a federated partner institution (BOOK-22's own
> `federated_faculty` engagement type, G18.5/FW6-09) no longer silently
> counts them toward STU's own DM 1154 requirement — the same professor
> could otherwise satisfy two universities' faculty-of-reference
> obligations at once, which the docente-di-riferimento rule exists to
> prevent. Recorded in both Books per BOOK-22 §11.5's own note that this is
> "a BOOK-26 concern surfaced by a BOOK-22 feature." Verified: C22.15,
> `tests/conformance/test_accreditation_guard.py` (4 tests) — adding a
> federated author leaves a program's Dtot/Ttot/gap/status verdict
> byte-for-byte unchanged, only the new separate figure moves.
...
**Persona × EKG-usage/RKG-maintenance (RFC-0002, BOOK-19 §1.2):** QA Officer's EKG usage is
coverage/traceability queries feeding AVA/DM1154 indicators; maintenance duty is conformance-check
verification against graph invariants (never mapping/policy approval itself). Auditor/CEV expert
gets read-only evidence/coverage audit views and no RKG-maintenance duty at all, by design — the
P-A read-only profile this Book already establishes for MUR/ANVUR auditors extends unchanged to
graph data.

Consume tutor **learning-gain** signals as quality evidence for AVA/DM1154 indicators (leading indicators of outcome attainment).
