...
| The nine agents | tutor/coach substrate (`learner_tutor_service`, GraphRAG, spaced repetition, Bloom quiz, plagiarism, credential service); **Discovery ✅ (STX-06, 2026-07-17)** — first agent config row + contract card (`ace_service.DISCOVERY_MOVE_BINDINGS`, `lifecycle_state="testing"`); **Academic Navigator ✅ (STX-07, 2026-07-19)** — `ace_service.seed_academic_navigator_agent`, `NAVIGATOR_MOVE_BINDINGS` (`replan_path`/`explain_concept`/`reframe_goal`), `lifecycle_state="testing"`; **Recognition ✅ (STX-12, 2026-07-19)** — `ace_service.seed_recognition_agent`, `RECOGNITION_MOVE_BINDINGS` (`recognize_prior`/`explain_concept`, propose-only per Ch. 3's card), `lifecycle_state="testing"`; **Assessment ✅ (STX-11, 2026-07-20)** — `ace_service.seed_assessment_agent`, `ASSESSMENT_MOVE_BINDINGS` (`generate_formative`/`give_feedback`/`check_retrieval`/`recommend_next`, all act-tier — summative work is a propose-tier skill outside the move cycle entirely, not a capped move), `lifecycle_state="testing"`; **Subject Tutor + Learning Coach ✅ (STX-10, 2026-07-22)** — `seed_subject_tutor_agent`/`seed_learning_coach_agent`, `SUBJECT_TUTOR_MOVE_BINDINGS` (`elicit`/`hint`/`worked_example`/`explain_concept`/`check_retrieval`/`give_feedback`/`challenge`) and `LEARNING_COACH_MOVE_BINDINGS` (`check_retrieval`/`encourage`/`recommend_next`/`celebrate`), both `lifecycle_state="testing"` — slugs `subject_tutor`/`learning_coach` match the pre-reserved `AGENT_ENTITLEMENTS` keys, not this row's own short-name convention (`tutor`/`coach`), which stays aspirational/unmaintained per every prior sprint's precedent; **Credential ✅ (STX-13, 2026-07-23)** — `ace_service.seed_credential_agent`, `CREDENTIAL_MOVE_BINDINGS` (`celebrate`/`recommend_next`, both act-tier — "reserved for high-stakes" is structural: certificate/degree issuance is a `credential_issuance_service` HITL proposal, never a move this agent's `run_cycle` can trigger, same boundary as Assessment's summative work), `lifecycle_state="testing"`; **Student Success + Career Advisor ✅ (STX-14/15, 2026-07-24)** — `seed_student_success_agent`/`seed_career_advisor_agent`, `STUDENT_SUCCESS_MOVE_BINDINGS` (`flag_risk`/`encourage`/`refer_to_human`/`abstain` — `flag_risk` files a `move_proposal_service` HITL proposal only on a genuine Decide-step confirmation, never a bypass) and `CAREER_ADVISOR_MOVE_BINDINGS` (`reframe_goal`/`recommend_next`/`explain_concept`/`refer_to_human`/`abstain` — ESCO gap analysis grounded only in a small v1 seed, honest `not_yet_available` outside it), both `lifecycle_state="testing"` — **all nine BOOK-10 Ch. 3 agents now have a real config row** | 🟡 | all nine delivered agents stay `testing` until their own NEW-02 harness gate run (none has run yet — no `deployed` transition for any of the nine; STX-11 backfilled Recognition's missing `backend/evals/scenario_bank/recognition.yaml`, a real gap found — Recognition could never have legitimately gated before that fix; STX-14/15 similarly found and fixed a missed `BANK_VERSION` bump from STX-13) |
