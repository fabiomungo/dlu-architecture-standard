...
| Two audit implementations coexist (`backend/domains/audit/` hash-chained, `backend/services/audit_service.py` non-hash-chained) | **Partially addressed (2026-08-08, hardening pass, not a numbered sprint)** — investigation found the legacy system wasn't just redundant, it was functionally near-completely broken (nearly every call site never awaited the async write, the internal execute/commit calls weren't awaited either, the blanket logging middleware had a permanent no-op placeholder, and the Postgres table was missing columns `log_action` has always assumed existed) — all fixed, and the new hash-chained fabric's REST API (fully built, never mounted) is now live with `require_admin()` gating added first (it had none). Still NOT consolidated into one system — `compliance_service.py`'s GDPR/report queries still read the legacy schema, and the hash-chained fabric's `tenant_id: Integer` (bridged via a `-1` sentinel) is still incompatible with the rest of the platform's UUID convention. See `docs/sprint_decisions_20260808_consolidation_debt_hardening.md`. | Full consolidation: future RFC, if ever prioritized |
