...
| Cross-tenant leakage fuzz (API + graph + streams + cache) | **✅ API dimension closed (2026-07-24)** — `tests/test_cross_tenant_leakage_fuzz.py` (9 tests). A read-only audit found and FIXED four real, exploitable IDOR/cross-tenant gaps in `backend/api/routes/ace.py` (`GET /memory`, `GET /proposals`, `GET/POST/DELETE /proposals/{id}` — the proposals gaps were genuinely cross-TENANT, since `move_proposal_service`'s Redis store carries no `tenant_id` at all). Graph (Neo4j) and Streams (event-mesh relay) dimensions remain honestly open — see `sprint_decisions_20260724_k3k4_hardening.md` §1 |
| Driver bulkhead chaos tests | **✅ partially closed (2026-07-24)** — fixed 6 genuinely broken pre-existing tests in `tests/chaos/test_pi2_disaster_recovery.py` (test rot: stale mocks, missing imports, a changed constructor signature, one test targeting a fictional API) plus 2 small defensive production fixes (`MoodleIntegrationService`/`FrappeIntegrationService.health_check()`); added the previously-entirely-missing "scoped degradation" proof (`test_moodle_down_does_not_block_other_systems_scoped_degradation`, `tests/test_enrollment_integration.py`) — none of the 27 existing tests had ever verified an unrelated system keeps working while one driver is down. Frappe CRM/Stripe/Keycloak/n8n/ESSE3 drivers, and a real circuit-breaker mechanism (none exists anywhere, retry+timeout only), remain open — see `sprint_decisions_20260724_k3k4_hardening.md` §2 |
