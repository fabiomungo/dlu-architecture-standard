...
|---------|--------------|--------|-----|
| Agent registry | `ai_agent_configs` (persona, scope, roles, knowledge_scope, max_turns, lifecycle, per-agent key, rate limits) | ✅ | `move_bindings`, `entitlements`, `envelope_defaults`, `calibration_ref` (ALTER, additive) 🔵 |
