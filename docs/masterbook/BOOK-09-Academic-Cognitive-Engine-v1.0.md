...
|---------|--------------|--------|-----|
| Engine skeleton | `agent_orchestrator_service` (workflow/step machine, authoring-side) | 🟡 | generalize to PDDAEL cycles (STX-06) |
| Agent/skill/preset registry | AI Management v2.0 (`ai_agent_configs`, `ai_skill_registry`, `ai_mcp_servers`, `ai_model_presets`) | ✅ | move-catalog binding per agent ⚪ |
| Gateway + presets + quotas | ACP Gateway (routing, fallback, virtual keys, budgets) | ✅ | escalation-ladder presets mapping 🔵 |
| Diagnose inputs | BKT, evidence (STX-08), behaviour aggregates, tutor context | 🟡 | LearnerStateAssessment type ⚪ |
| Grounded realization | GraphRAG, RAG service, mock LLM (testing) | ✅ | grounding-mandatory enforcement per move class 🔵 |
| HITL queue | AI proposals/approval routes, `ai_governance_service` | ✅ | deferral triggers wiring ⚪ |
| Traces | AI generation audit tables | 🟡 | cycle trace schema (Ch. 8) ⚪ |
| Missions | Celery + beat | ✅ | cognitive budget scheduler ⚪ |
| Calibration | — | ⚪ | per-move calibration store + steward alerts (BOOK-11 eval harness) |
| One-voice composition | — | ⚪ | STX-06 Brain layer |
