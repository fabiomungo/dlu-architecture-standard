...
| Socratic tutoring (P4) | `tutor_dialogue_service.py` + Subject Tutor agent (✅ STX-10, 2026-07-22 — see BOOK-09/BOOK-11 Annex A) | ✅ | grounding is enrolled-course-scope-only via `explain_concept`'s existing `grounding_required` check; scaffolding fade beyond the elicit→hint→worked_example ladder (e.g. hint specificity itself narrowing as mastery grows) remains a documented follow-up ⚪ |
